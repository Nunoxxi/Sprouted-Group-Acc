...
const entitySeed = [
  {
    ...
    name: 'Sprouted Roots',
    type: 'NFP',
    template: ngoAccounts,
  },
  {
    code: 'SG002',
    name: 'Sprouted Crafts',
    type: 'LLC',
    template: [...manufacturingAccounts, ...sharedTaxAccounts],
  },
  {
    code: 'SG003',
    name: 'Oikazi',
...
async function main() {
  for (const item of entitySeed) {
    let entity = await prisma.entity.findUnique({ where: { code: item.code } });
    ...
    for (const template of item.template) {
      const code = `${template.code}`;
      if (accountCodes.has(code)) continue;
      accountCodes.add(code);

      const parentMatch = await prisma.account.findFirst({
        where: {
          entityId: entity.id,
      ...
      await prisma.account.upsert({
        ...
        update: {
          name: template.name,
          type: normalizeAccountType(template.type),
