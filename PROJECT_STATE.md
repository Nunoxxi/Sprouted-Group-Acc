...
- Every Server Function is reachable by direct POST with no authentication (see gap 3).
...
3. **Auth / users — do this first.** `currentUserName` is a placeholder, and every Server Function in `src/app/actions/documents.ts` is reachable by direct POST with no authentication. Anyone who can reach the URL can post, void and file periods for any entity. Not deployable anywhere untrusted until this is done.
